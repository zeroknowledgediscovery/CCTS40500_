...
def getData(meta, X_raw, tgt, future):
	...
	so we predict for all three variables, VAR, ASSAULT, PROPERTY
	"""
	...
	X_train = X[:730 - future]
	Y_train = Y[future: 730]

	X_test = X[730 - future: -future]
	Y_test = Y[730:]
...
def train(X_train, Y_train, epochs=200):
	...
	# One output layers
	model.add(TimeDistributed(Dense(units=3, activation='sigmoid')))
...
def Analysis(Y_test, prediction, figTitle, dfName):
	...
	resDf.index = pd.date_range(start='1/1/2016', end='12/31/2016')
	resDf.to_csv(dfName)
